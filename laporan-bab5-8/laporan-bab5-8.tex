...
\lstinputlisting[language=SQL, caption={prisma/schema.prisma -- Skema Database}]{source-code/prisma/schema.prisma}
...
\lstinputlisting[language=JavaScript, caption={src/lib/authMiddleware.ts -- Autentikasi JWT \& RBAC}]{source-code/src/lib/authMiddleware.ts}

\subsection{API Login}

Berikut adalah endpoint API untuk autentikasi pengguna:

\lstinputlisting[language=JavaScript, caption={src/app/api/auth/login/route.ts -- API Login}]{source-code/src/app/api/auth/login/route.ts}

\subsection{API Kelola Karyawan}

Berikut adalah endpoint API untuk manajemen data karyawan (\textit{CRUD}):

\lstinputlisting[language=JavaScript, caption={src/app/api/karyawan/route.ts -- API Karyawan (GET/POST)}]{source-code/src/app/api/karyawan/route.ts}

\subsection{API Submit Penilaian}

Berikut adalah endpoint API untuk submit jawaban penilaian 360\textdegree{}:

\lstinputlisting[language=JavaScript, caption={src/app/api/penilaian/submit/[formId]/route.ts -- Submit Penilaian}]{source-code/src/app/api/penilaian/submit/[formId]/route.ts}
...
\lstinputlisting[language=JavaScript, caption={src/app/api/penilaian/hasil/[karyawanId]/[periodeId]/route.ts -- Hasil Penilaian}]{source-code/src/app/api/penilaian/hasil/[karyawanId]/[periodeId]/route.ts}
...
\lstinputlisting[language=JavaScript, caption={src/app/api/laporan/route.ts -- API Laporan Evaluasi}]{source-code/src/app/api/laporan/route.ts}

\subsection{API Talent Mapping (9-Box)}

Berikut adalah endpoint API untuk pemetaan talenta 9-Box Matrix:

\lstinputlisting[language=JavaScript, caption={src/app/api/talent/route.ts -- API Talent Mapping}]{source-code/src/app/api/talent/route.ts}

\subsection{Halaman Login}

Berikut adalah kode halaman login dengan demo account selector:

\lstinputlisting[language=HTML, caption={src/app/page.tsx -- Halaman Login}]{source-code/src/app/page.tsx}

\subsection{Dashboard Layout}

Berikut adalah kode layout dashboard yang mencakup sidebar dan header:

\lstinputlisting[language=HTML, caption={src/app/dashboard/layout.tsx -- Layout Dashboard}]{source-code/src/app/dashboard/layout.tsx}

\subsection{Komponen Sidebar}

Berikut adalah komponen sidebar navigasi yang menyesuaikan berdasarkan peran pengguna:

\lstinputlisting[language=HTML, caption={src/components/Sidebar.tsx -- Sidebar Navigasi}]{source-code/src/components/Sidebar.tsx}

\subsection{Seed Database}

Berikut adalah skrip untuk mengisi database dengan data awal untuk demonstrasi:

\lstinputlisting[language=JavaScript, caption={prisma/seed.ts -- Seeder Database}]{source-code/prisma/seed.ts}

\subsection{Environment Configuration}

Berikut adalah konfigurasi environment variable yang digunakan oleh sistem:

\lstinputlisting[caption={.env -- Environment Variables}]{source-code/.env}

\subsection{Package Dependencies}

Berikut adalah daftar dependensi utama yang digunakan dalam pengembangan sistem:

\lstinputlisting[language=JSON, caption={package.json -- Dependensi Aplikasi}]{source-code/package.json}
