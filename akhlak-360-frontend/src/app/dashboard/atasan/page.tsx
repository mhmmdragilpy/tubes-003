...
import { CheckCircle2, XCircle, Users, ClipboardList, UserCheck } from "lucide-react";
import {
...
const daftarPeer = [
  { id: 1, karyawan: "Ahmad Fauzi", peer: "Hendri Wijaya", hubungan: "Peer", status: "PENDING" },
  { id: 2, karyawan: "Ahmad Fauzi", peer: "Ratna Sari", hubungan: "Peer", status: "APPROVED" },
  { id: 3, karyawan: "Rizky Pratama", peer: "Siti Rahmawati", hubungan: "Peer", status: "PENDING" },
  { id: 4, karyawan: "Mega Putri", peer: "Dewi Anggraini", hubungan: "Peer", status: "REJECTED" },
];

export default function AtasanDashboard() {
  const [peerList, setPeerList] = useState(daftarPeer);

  const handleApprove = (id: number) => {
    setPeerList((prev) => prev.map((p) => (p.id === id ? { ...p, status: "APPROVED" } : p)));
  };
  const handleReject = (id: number) => {
    setPeerList((prev) => prev.map((p) => (p.id === id ? { ...p, status: "REJECTED" } : p)));
  };
