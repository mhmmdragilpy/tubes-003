...
import { Download, FileText, Loader2, Trophy, ArrowUpRight, Award, ChevronDown } from "lucide-react";
...
export default function LaporanPage() {
  ...
  const handleExportPDF = () => {
    ...
    // Header
    doc.setFontSize(20);
    doc.text("Laporan Evaluasi 360° AKHLAK", 14, 22);
    ...
    const tableColumn = ["Peringkat", "NIP", "Nama", "Departemen", "Score Self", "Score Atasan", "Score Peer", "Final Score", "Predikat"];
    ...
    doc.save(`Laporan_AKHLAK_${periodeName ? periodeName.replace(/\s+/g, '_') : 'Export'}.pdf`);
  };
  ...
  return (
    <div className="space-y-6 pb-10">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-800 tracking-tight">Laporan Evaluasi</h1>
          <p className="text-slate-500 mt-1 font-medium">Hasil kalkulasi akhir Penilaian AKHLAK 360.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="card p-10 flex flex-col items-center justify-center text-center">
           ...
           <p className="text-slate-500 mt-2 max-w-md">Pastikan periode telah berjalan dan karyawan sudah men-submit form penilaian.</p>
        </div>
      ) : (
              ...
              <table className="w-full text-left text-sm text-slate-600">
                <thead className="bg-slate-50 border-b border-slate-200 text-xs uppercase font-bold tracking-wider text-slate-500">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-center">Score Self (10%)</th>
                    <th className="px-6 py-4 text-center">Score Atasan (60%)</th>
                    <th className="px-6 py-4 text-center">Score Peer (30%)</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {data.map((k, index) => (
                    <tr key={k.id_karyawan} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-center font-medium">{k.scoreSelf}</td>
                      <td className="px-6 py-4 text-center font-medium">{k.scoreAtasan}</td>
                      <td className="px-6 py-4 text-center font-medium">{k.scorePeer}</td>
